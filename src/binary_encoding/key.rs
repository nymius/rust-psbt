...
pub(crate) use encoding::{
    ...
    CompactSizeEncoder, Decoder, Encoder, Encoder3,
};
...
/// TODO
pub struct PsbtKeyEncoder<'a>(KeyEncoder<'a>);

impl PsbtEncoder for PsbtKeyEncoder<'_> {
    fn current_chunk(&self) -> &[u8] { self.0.current_chunk() }

    fn advance(&mut self) -> bool { self.0.advance() }
}

impl PsbtEncodable for Key {
    type Encoder<'a>
        = PsbtKeyEncoder<'a>
    where
        Self: 'a;
    ...
    fn encoder(&self) -> Self::Encoder<'_> {
        let encoded_size = CompactSizeEncoder::encoded_size(self.ttype as usize);
        PsbtKeyEncoder(KeyEncoder::new(Encoder3::new(
            CompactSizeEncoder::new(self.data.len() + encoded_size),
            CompactSizeEncoder::new(self.ttype as usize),
...
pub struct PsbtKeyDecoder {
    state: PsbtKeyDecoderState,
}

impl Default for PsbtKeyDecoder {
    fn default() -> Self { Self::new() }
}

impl PsbtKeyDecoder {
    /// Constructs a new [`PsbtKeyDecoder`].
    pub const fn new() -> Self {
        Self { state: PsbtKeyDecoderState::KeyLength(ByteVecDecoder::new()) }
    }
}

/// An error consensus decoding a PSBT [`Key`]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PsbtKeyDecoderError(PsbtKeyDecoderErrorInner);

#[derive(Debug, Clone, PartialEq, Eq)]
enum PsbtKeyDecoderErrorInner {
...
impl alloc::fmt::Display for PsbtKeyDecoderError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        use PsbtKeyDecoderErrorInner as E;
...
impl PsbtDecodable for Key {
    type Decoder = PsbtKeyDecoder;
    fn decoder() -> Self::Decoder { PsbtKeyDecoder::new() }
}

impl PsbtDecoder for PsbtKeyDecoder {
    type Output = Key;
    type Error = PsbtKeyDecoderError;

    #[inline]
    fn push_bytes(&mut self, bytes: &mut &[u8]) -> Result<bool, Self::Error> {
        use PsbtKeyDecoderError as E;
        use PsbtKeyDecoderErrorInner as Inner;
        use PsbtKeyDecoderState as State;
        ...
        loop {
            ...
            match core::mem::replace(&mut self.state, State::Errored) {
                ...
                State::KeyType(type_and_data, decoder) => {
                    let ttype_usize = decoder.end().map_err(|e| E(Inner::Type(e)))?;

                    let ttype = u64::try_from(ttype_usize).expect("the maximum encodable value in a compact size unsigned integer fits within u64");

                    let data = (*type_and_data).to_vec();

                    self.state = State::Done(Key { ttype, data });
                }
                State::Done(..) => {
                    return Ok(false);
                }
...
#[cfg(test)]
mod tests {
    use super::{Key, PsbtDecodable, PsbtDecoder, PsbtEncodable, PsbtEncoder, PsbtKeyDecoder, Vec};
    ...
    #[test]
    fn roundtrip() {
        let original = Key { ttype: 0x06, data: vec![0x01, 0x02, 0x03, 0x04] };
    ...
    mod encode {
        ...
        #[test]
        fn keytype_only() {
            let key = Key { ttype: 0x00, data: vec![] };
        ...
        #[test]
        fn key_with_keydata() {
            let key = Key { ttype: 0x02, data: vec![0x01, 0x02, 0x03] };
            let encoded = encode_key(key);
    ...
    mod decode {
        ...
        fn key_with_keydata() {
            ...
            assert_eq!(key.ttype, 0x02);
            assert_eq!(key.data, vec![0x01, 0x02, 0x03]);
        }
        ...
        fn early_end_while_decoding_length() {
            let decoder = PsbtKeyDecoder::new();
        ...
        fn early_end_while_decoding_type() {
            let mut decoder = PsbtKeyDecoder::new();
